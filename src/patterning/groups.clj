...
;;; Simple transforms
(defn scale-group ([val group] (into [] (map (partial sshapes/scale val) group )))   )

(defn translate-group  [dx dy group] (into [] (map (partial sshapes/translate dx dy) group))  )

(defn translate-group-to [x y group] (translate-group (- x) (- y) group) )  

(defn h-reflect-group [group] (into [] (map sshapes/h-reflect group) ) )
(defn v-reflect-group [group] (into [] (map sshapes/v-reflect group) ) )

(defn stretch-group [sx sy group] (into [] (map (partial sshapes/stretch sx sy) group)))
(defn rotate-group [da group] (into [] (map (partial sshapes/rotate da) group)))

(defn wobble-group [noise group] (into [] (map (partial sshapes/wobble noise) group)))
